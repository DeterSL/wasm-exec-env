...
use wasmtime_wasi::p2::{OutputStream, StreamError, Pollable, InputStream};
use tokio::{sync::{mpsc, oneshot}};
use wasmtime_wasi::runtime::poll_noop;
use std::task::{Context, Poll};

use super::{bindings::detersl::http_api::types::ErrorCode, types::FieldMap};
...
impl HostIncomingBody {
    ...
    pub fn take_stream(&mut self) -> Option<HostIncomingBodyStream> {
        ...
        let body = match std::mem::replace(&mut self.body, IncomingBodyState::InBodyStream(rx)) {
...
impl HostIncomingBodyStream {
    fn record_frame(&mut self, frame: Option<Result<Frame<Bytes>, ErrorCode>>) {
        match frame {
            Some(Ok(frame)) => match frame.into_data() {
                ...
                Err(trailers) => {
                    ...
                    let tx = match std::mem::replace(&mut self.state, IncomingBodyStreamState::Closed) {
...
impl Drop for HostIncomingBodyStream {
    fn drop(&mut self) {
        ...
        let prev = std::mem::replace(&mut self.state, IncomingBodyStreamState::Closed);
...
impl HostOutgoingBody {
                        ...
                        if let Some(mut finish_receiver) = self.as_mut().finish_receiver.take() {
                            match Pin::new(&mut finish_receiver).poll(cx) {
                                ...
                                Poll::Ready(Ok(message)) => match message {
                                    ...
                                    FinishMessage::Abort => {
                                        Poll::Ready(Some(Err(ErrorCode::HttpProtocolError)))
                                    }
                                },
    ...
    pub fn finish(mut self, trailers: Option<FieldMap>) -> Result<(), ErrorCode> {
...
impl StreamContext {
    ...
    pub fn as_body_size_error(&self, size: u64) -> ErrorCode {
        match self {
            StreamContext::Request => ErrorCode::HttpRequestBodySize(Some(size)),
            StreamContext::Response => ErrorCode::HttpResponseBodySize(Some(size)),
        }
    }
}
